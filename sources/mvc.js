...
compliant("SiteMap", window.SiteMap = {
     ...
     * - Character - always embedded between the characters: a-z _ 0-9, it can
     *   not be used at the beginning and end
     ...
     * loaded and triggers the renderer again if the path requested with the URL
     * differs.
     */        
    get location() {
    ...
    /**
     * Checks a path against existing/registered permission methods. A path is
     * allowed only if all permission methods return true.
     * @param  path to check (URL is also supported, only the hash is used here
     *     and the URL itself is ignored)
    ...
    /**
     * Determines the real existing path according to the SiteMap. The methods
     * distinguish between absolute, relative and functional paths. The
     * functionalities remain unchanged. Absolute and relative paths are
     * balanced. Relative paths are balanced on the basis of the current
     * location. All paths are checked against the SiteMap. Invalid paths are
     * searched for a valid partial path. To do this, the path is shortened
     * piece by piece. If no valid partial path can be found, the root is
     * returned. Without passing a path, the current location is returned.
     * @param  path to check - optional (URL is also supported, only the hash is
    ...
    /**
     * Navigates to the given path, if it exists in the SiteMap. All paths are
     * checked against the SiteMap. Invalid paths are searched for a valid
     * partial path. To do this, the path is shortened piece by piece. If no
     * valid partial path can be found, the root is the target.
     * 
    ...
    /**
     * Forwards to the given path, if it exists in the SiteMap. All paths are
     * checked against the SiteMap. Invalid paths are searched for a valid
     * partial path. To do this, the path is shortened piece by piece. If no
     * valid partial path can be found, the root is the target.
     * 
     * In difference to the navigate method, the forwarding is executed
     * directly, instead the navigate method triggers asynchronous forwarding by
    ...
    /**
     * Checks whether a path or sub-path is currently being used. This is used
     * to show/hide composites depending on the current location.
     * @param  path
    ...
    accept(path) {
        ...
        // (Partial)faces always have the higher priority for facets. If nothing
        // can be determined, there cannot be a valid path.
        ...
        // already cause a false termination of this method (return false) and
        // do not have to be checked here anymore.
     ...
     * meta object. The keys (string) correspond to the paths, the values are
     * arrays with the valid facets for a path.
     * 
     *     sitemap = {
     ...
     *     };
     *     
     * The method has different signatures and can be called several times. If
     * the method is called more than once, the configurations are concatenated
     * and existing values in the configuration are overwritten.
     * 
     ...
     *     ----
     * The first configuration describes the SiteMap as a meta object. The meta
     * object defines all available paths and facets for the face flow.    
     *     
     ...
     * concepts and can be used to check and manipulate paths. Several permit
     * methods can be registered. All requested paths pass through the permit
     * method(s). This can decide what happens to the path. From the permit
     * method a return value is expected, which can have the following
     * characteristics:
     * 
     *     true:
     ...
     *     Acceptor:
     *     ----
     * Acceptors work in a similar way to permit methods. In difference, permit
     * methods are called for each path and acceptors are only called for those
     * that match the RegExp pattern. Also from the permit method a return value
     * is expected, which can have the following characteristics -- see SiteMap
...
/**
 * Rendering filter for all composite elements. The filter causes an additional
 * condition for the SiteMap to be added to each composite element that the
 * renderer determines. This condition is used to show or hide the composite
 * elements in the DOM to the corresponding virtual paths. The elements are
 * identified by the serial. The SiteMap uses a map (serial/element) as cache
 * for fast access. The cleaning of the cache is done by a MutationObserver.
 */
...
/**
 * Registration of the attribute 'x' for hardening. The attribute is therefore
 * more difficult to manipulate in markup.
 */
Composite.customize("@ATTRIBUTES-STATICS", "static");

/**
 * Established a listener that listens when the page loads. The method initiates
 * the initial usage of the path.
 */
window.addEventListener("load", () => {
    
    // When clicking on a link with the current path, the focus must be set back
    // to face/facet, as the user may have scrolled on the page. However, this
    // is only necessary if face + facet have not changed. In all other cases
    // the Window-HashChange-Event does the same
...
window.addEventListener("hashchange", (event) => {
    ...
    // Rendering is not necessary because the face/facet does not change or the
    ...
    // If window.location.hash, SiteMap.location and new URL match, no update or
    ...
    // If the permission is not confirmed, will be forwarded to the next higher
    // known/permitted path, based on the requested path. Alternatively, the
    // permit methods can also supply a new target, which is then jumped to.
    ...
    // The locations are updated synchronously. The possible triggering of the
    // hashchange-event can be ignored, because SiteMap.location and
    // window.location.hash are the same and therefore no update or rendering is
    // triggered.
    ...
    // was hidden before rendering or did not exist. For this purpose, the
    // setter must be called in window.location.
