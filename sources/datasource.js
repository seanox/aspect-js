...
compliant("DataSource", {
     ...
     * The locator syntax is based on the URL syntax. Only the parts schema and
     * path are used. A path segment begins with a word character _ a-z 0-9,
     * optionally more word characters and additionally - can follow, but can
     * not end with the - character. Paths are separated by the / character.
     */
     ...
     * mix. The result as a DocumentFragment. Optionally, a meta object or a map
     * with parameters for the XSLTProcessor can be passed.
    ...
    transform(xml, style, meta) {
        ...
        // The escape attribute converts text to HTML. Without the escape
        // attribute, HTML tag symbols < and > are masked and output as text.
        ...
        // with !DOCTYPE + !ENTITY. Therefore the document is copied so that the
     ...
     * or map with parameters for the XSLTProcessor can be passed. When using
     * the transformation, the return type changes to a  DocumentFragment.
     * @param  locators  locator
     * @param  transform locator of the transformation style
     *     With the boolean true, the style is derived from the locator by using
     *     the file extension xslt.
     * @param  meta      optional parameters for the XSLTProcessor
