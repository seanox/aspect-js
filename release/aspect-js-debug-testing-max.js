...
(() => {
    ...
    const _filter = window.___filter = (...levels) => {
        const chain = [];
    ...
    const _populate = window.___populate = (namespace, level) => {
     ...
     */
    const _appendChild = window.___appendChild = Element.prototype.appendChild;
    ...
    compliant("Math.serial", () =>
        _serial.toString());
    const _offset = window.___offset = -946684800000;
    const _serial = window.___serial = {timing:Date.now() + _offset, number:0,
        toString() {
...
(() => {
    ...
    const _import = window.___import = (...imports) => {
    ...
    const _export = window.___export = (...exports) => {
        // Because it is an internal method, an additional validation of the
        // exports as data structure was omitted.
    ...
    const _use = window.___use = (...uses) => {
        uses.forEach(use => Namespace.use(use));
    }

    const _tolerate = window.___tolerate = (invocation) => {
...
(() => {
    ...
    const _cache = window.___cache = new Map();
    ...
    const _fill = window.___fill = (expression, patches) =>
     ...
     */
    const _parse = window.___parse = (type, expression, patches = []) => {
...
(() => {
    ...
    const _asynchronous_queue = window.___asynchronous_queue = [];

    /** Storage for variables with the page scope */
    const _render_context_scope = window.___render_context_scope = [];

    /** Storage for dynamic/temporary variables with the page scope */
    const _render_context_stack = window.___render_context_stack = [];
     ...
     */
    const _render_context_workspace = window.___render_context_workspace = [];
     ...
     *     RENDER        VALIDATE
     */
    const _statics = window.___statics = new Set();

    /** Map for custom tags (key:tag, value:function) */
    const _macros = window.___macros = new Map();

    /** Map for custom selectors (key:hash, value:{selector, function}) */
    const _selectors = window.___selectors = new Map();

    /** Set with interceptor and their registered listeners */
    const _interceptors = window.___interceptors = new Set();

    /** Map with events and their registered listeners */
    const _listeners = window.___listeners = new Map();
     ...
     */
    const _models = window.___models = new Set();

    const _recursion_detection = window.___recursion_detection = (element) => {
     ...
     */
    const _mount_locate = window.___mount_locate = (element) => {
     ...
     * @throws {Error} In case of an invalid IDs for composites and elements
     */
    const _mount_lookup = window.___mount_lookup = (element) => {
     ...
     */
    const _request_open = window.___request_open = XMLHttpRequest.prototype.open;
    ...
    window.addEventListener("load", () => {
        ...
        const _cleanup = window.___cleanup = (node) => {
...
(() => {
    ...
    const _datasource = window.___datasource = [DataSource.data];

    const _localize = window.___localize = DataSource.localize;

    const _load = window.___load = (data) => {
        const map = new Map();
...
(() => {
     ...
     */
    const _secret = window.___secret = Math.serial();
     ...
     */
    const _register = window.___register = new WeakMap();

    const _reactive = window.___reactive = (object) => {
...
(() => {
    ...
    const _interceptors = window.___interceptors = [];

    // Array with the path history (optimized)
    const _history = window.___history = [];
    ...
    const _locate = window.___locate = (location) => {
        if (location === null)
            return null;
     ...
     */
     const _lookup = window.___lookup = (lookup) => {
    ...
    const _render = window.___render = (element, focus = false) => {
...
(() => {
    ...
    const _activate = window.___activate = () => {
        ...
        const _stack = window.___stack = new Set();
        ...
        (() => {
            ...
            const _output = window.___output = {log:"", warn:"", error:"", info:""};
            ...
            const _listeners = window.___listeners = new Set();
